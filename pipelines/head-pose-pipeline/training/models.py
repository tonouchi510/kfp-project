...
class BaseFSANet(object):
    ...
    def ssr_G_model_build(self, img_inputs):
        ...
        feat_s3_pre = tf.keras.layers.AveragePooling2D((2,2))(feat_s3_pre) # make sure (8x8x64) feature maps 
    
        return tf.keras.models.Model(inputs=img_inputs,outputs=[feat_s1_pre,feat_s2_pre,feat_s3_pre], name='ssr_G_model')
    ...
    def ssr_F_model_build(self, feat_dim, name_F):
        ...
        def _process_input(stage_index, stage_num, num_classes, input_s_pre):            
            ...
            local_s = tf.keras.layers.Dense(units=num_classes, activation='tanh', name=f'local_delta_stage{stage_index}')(feat_local_s)
            
            feat_pred_s = FeatSliceLayer(8,16)(input_s_pre)            
            feat_pred_s = tf.keras.layers.Dense(stage_num*num_classes,activation='relu')(feat_pred_s) 
        ...
        return tf.keras.models.Model(inputs=[input_s1_pre,input_s2_pre,input_s3_pre],outputs=[pred_s1,pred_s2,pred_s3,delta_s1,delta_s2,delta_s3,local_s1,local_s2,local_s3], name=name_F)
    ...
    def ssr_FC_model_build(self, feat_dim, name_F):
        ...
        def _process_input(stage_index, stage_num, num_classes, input_s_pre):
            feat_delta_s = tf.keras.layers.Dense(2*num_classes,activation='tanh')(input_s_pre)
            delta_s = tf.keras.layers.Dense(num_classes,activation='tanh',name=f'delta_s{stage_index}')(feat_delta_s)

            feat_local_s = tf.keras.layers.Dense(2*num_classes,activation='tanh')(input_s_pre)
            local_s = tf.keras.layers.Dense(units=num_classes, activation='tanh', name=f'local_delta_stage{stage_index}')(feat_local_s)

            feat_pred_s = tf.keras.layers.Dense(stage_num*num_classes,activation='relu')(input_s_pre) 
            pred_s = tf.keras.layers.Reshape((num_classes,stage_num))(feat_pred_s)     
    ...
    def ssr_feat_S_model_build(self, m_dim):
        ...
        if self.is_varS_model:
            ...
            feat_preS = tf.keras.layers.Conv2D(1,(1,1),padding='same',activation='sigmoid')(input_preS)        

        feat_preS = tf.keras.layers.Reshape((-1,))(feat_preS)        

        SR_matrix = tf.keras.layers.Dense(m_dim*(self.map_xy_size*self.map_xy_size*3),activation='sigmoid')(feat_preS)
        SR_matrix = tf.keras.layers.Reshape((m_dim,(self.map_xy_size*self.map_xy_size*3)))(SR_matrix)
        
        return tf.keras.models.Model(inputs=input_preS,outputs=[SR_matrix,feat_preS],name='feat_S_model')
    ...
    def ssr_S_model_build(self, num_primcaps, m_dim):
        ...
        feat_pre_concat = tf.keras.layers.Concatenate()([feat_s1_preS,feat_s2_preS,feat_s3_preS])
        SL_matrix = tf.keras.layers.Dense(int(num_primcaps/3)*m_dim,activation='sigmoid')(feat_pre_concat)
        ...
        feat_s1_pre = tf.keras.layers.Reshape((self.map_xy_size*self.map_xy_size,64))(input_s1_preS)
        feat_s2_pre = tf.keras.layers.Reshape((self.map_xy_size*self.map_xy_size,64))(input_s2_preS)
        feat_s3_pre = tf.keras.layers.Reshape((self.map_xy_size*self.map_xy_size,64))(input_s3_preS)

        feat_pre_concat = tf.keras.layers.Concatenate(axis=1)([feat_s1_pre, feat_s2_pre, feat_s3_pre])
        ...
        primcaps = tf.keras.layers.Concatenate(axis=1)([primcaps_s1,primcaps_s2,primcaps_s3])

        return tf.keras.models.Model(inputs=[input_s1_preS, input_s2_preS, input_s3_preS],outputs=primcaps, name='ssr_S_model')
    ...
    def ssr_noS_model_build(self, **kwargs):        
        ...
        primcaps_s1 = tf.keras.layers.Reshape((self.map_xy_size*self.map_xy_size,64))(input_s1_preS)
        primcaps_s2 = tf.keras.layers.Reshape((self.map_xy_size*self.map_xy_size,64))(input_s2_preS)
        primcaps_s3 = tf.keras.layers.Reshape((self.map_xy_size*self.map_xy_size,64))(input_s3_preS)
...
class BaseCapsuleFSANet(BaseFSANet):
    ...
    def ssr_aggregation_model_build(self, shape_primcaps):
        ...
        return tf.keras.models.Model(inputs=input_primcaps,outputs=[feat_s1_div,feat_s2_div,feat_s3_div], name='ssr_Cap_model')     
...
class BaseNetVLADFSANet(BaseFSANet):
    ...
    def ssr_aggregation_model_build(self, shape_primcaps):
        ...
        agg_feat = tf.keras.layers.Reshape((self.num_capsule,self.dim_capsule))(agg_feat)
        ...
        return tf.keras.models.Model(inputs=input_primcaps,outputs=[feat_s1_div,feat_s2_div,feat_s3_div], name='ssr_Agg_model')                
...
class BaseMetricFSANet(BaseFSANet):
    ...
    def ssr_aggregation_model_build(self, shape_primcaps):
        ...
        return tf.keras.models.Model(inputs=input_primcaps,outputs=[feat_s1_div,feat_s2_div,feat_s3_div], name='ssr_Metric_model')     
